...
//  SXAlertView
//
//  Created by Bailing Cloud on 2017/10/10.
...
typedef void (^SXAlertViewClickTextFieldsBlock) (SXAlertView *alertView, NSInteger buttonIndex, NSArray<NSString *> *texts);

CGFloat const marginX = 16;
CGFloat const marginY = 20;

static SXAlertView *globalAlertView;
...
@property(nonatomic, copy)SXAlertViewClickTextFieldsBlock clickTextFieldsBlock;
...
- (instancetype)initWithTitle:(nullable NSString *)title placeholder:(nullable  NSArray<NSString *> *)placeholder cancelButtonTitle:(nullable NSString *)cancelButtonTitle otherButtonTitle:(nullable NSString *)otherButtonTitle clickButtonBlock:(nullable SXAlertViewClickTextFieldsBlock)block{
    if (self = [super init]) {
        // 1.设置数据
        self.title = title;
        self.clickTextFieldsBlock = block;
        // 2.初始化UI
        [self setupDefault];
        // 3.添加子视图
        [self.contentView addSubview:self.labelTitle];
        [self.contentView addSubview:self.textField];
        [self.contentView addSubview:self.textFieldSec];
        [self.contentView addSubview:self.textFieldThird];
 
        if (placeholder && placeholder.count > 0) {
            self.textField.text = placeholder[0];
            self.textFieldSec.text = placeholder[1];
            self.textFieldThird.text = placeholder[2];
        }
        self.textField.placeholder = @"CMP Server";
        self.textFieldSec.placeholder = @"Token";
        self.textFieldThird.placeholder = @"AppKey";
        
        // 4.初始化Button
        CGFloat buttonY = CGRectGetMaxY(self.textFieldThird.frame) + marginY;
 ...
 + (void)showWithTitle:(nullable NSString *)title placeholder:(nullable  NSArray<NSString *> *)placeholder cancelButtonTitle:(nullable NSString *)cancelButtonTitle otherButtonTitle:(nullable NSString *)otherButtonTitle clickButtonBlock:(nullable void (^)(SXAlertView *alertView, NSInteger buttonIndex, NSArray<NSString *>  *text))block{
 
     if (globalAlertView) {
         return;
     }
    globalAlertView = [[SXAlertView alloc]initWithTitle:title placeholder:placeholder  cancelButtonTitle:cancelButtonTitle otherButtonTitle:otherButtonTitle clickButtonBlock:block];
...
- (instancetype)initWithTitle:(nullable NSString *)title image:(nullable UIImage *)image cancelButtonTitle:(nullable NSString *)cancelButtonTitle otherButtonTitle:(nullable NSString *)otherButtonTitle clickButtonBlock:(nullable void (^)(SXAlertView *alertView, NSInteger buttonIndex))block{
    if (self = [super init]) {
        ...
        // 4.初始化Button
        CGFloat buttonY = CGRectGetMaxY(self.imageIcon.frame) + marginY;
        [self setupButtonWithbuttonY:buttonY cancelButtonTitle:cancelButtonTitle otherButtonTitle:otherButtonTitle];
...
- (instancetype)initTextViewWithTitle:(nullable NSString *)title placeholder:(nullable NSString *)placeholder cancelButtonTitle:(nullable NSString *)cancelButtonTitle otherButtonTitle:(nullable NSString *)otherButtonTitle clickButtonBlock:(nullable void (^)(SXAlertView *alertView, NSInteger buttonIndex, NSString *text))block{
    if (self = [super init]) {
        ...
        // 4.初始化Button
        CGFloat buttonY = CGRectGetMaxY(self.textView.frame) + marginY;
        [self setupButtonWithbuttonY:buttonY cancelButtonTitle:cancelButtonTitle otherButtonTitle:otherButtonTitle];
        // 5.添加通知
        [self addObserverKeyboardNotification];
    }
...
- (void)setupButton{
    CGFloat buttonY = CGRectGetMaxY(self.labelMessage.frame) + marginY;
...
- (void)setupButtonWithbuttonY:(CGFloat)buttonY cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitle:(NSString *)otherButtonTitle{
    UIButton *buttonCancel = [self buttonWithFrame:CGRectMake(0,buttonY, self.contentWidth/2, 50) title:cancelButtonTitle target:self action:@selector(clickCancel:)];
    UIButton *buttonOther = [self buttonWithFrame:CGRectMake(self.contentWidth/2, buttonY, self.contentWidth/2, 50) title:otherButtonTitle target:self action:@selector(clickOther:)];
    [self.contentView addSubview:buttonOther];
    [self.contentView addSubview:buttonCancel];
    
    self.contentView.frame = CGRectMake(0, 0, self.contentWidth, buttonY+50);
    self.contentView.center = self.center;
}
...
//    }
    ...
    if (self.clickTextFieldsBlock) {
        NSMutableArray *array = [NSMutableArray array];
        [array addObject:self.textField.text];
        [array addObject:self.textFieldSec.text];
        [array addObject:self.textFieldThird.text];
        self.clickTextFieldsBlock(self, 1, array);
    }
...
- (UIView *)contentView{
    if (!_contentView) {
        ...
        _contentView.center = CGPointMake(self.screenWidth/2, self.screenHeight/2*0.6);
...
- (UITextView *)labelMessage
{
    if (!_labelMessage) {
        ...
        _labelMessage.frame = CGRectMake(marginX, marginY, self.contentWidth-2*marginX, 20);
...
- (UITextField *)textField
{
    if (!_textField) {
        ...
        _textField.frame = CGRectMake(marginX, self.labelMessage.frame.origin.y+self.labelMessage.frame.size.height+marginY, self.contentWidth-2*marginX, 44);
...
- (UITextField *)textFieldSec
{
    if (!_textFieldSec) {
        _textFieldSec = [[UITextField alloc]init];
        _textFieldSec.frame = CGRectMake(marginX, marginY + _textField.frame.origin.y+_textField.frame.size.height, self.contentWidth-2*marginX, 44);
        _textFieldSec.textColor = [UIColor blackColor];
        _textFieldSec.font = [UIFont systemFontOfSize:15];
        _textFieldSec.borderStyle = UITextBorderStyleRoundedRect;
        _textFieldSec.clearButtonMode = UITextFieldViewModeWhileEditing;

        [_textFieldSec addTarget:self action:@selector(clickTextChanged:) forControlEvents:UIControlEventEditingChanged];
    }
    return _textFieldSec;
}

- (UITextField *)textFieldThird
{
    if (!_textFieldThird) {
        _textFieldThird = [[UITextField alloc]init];
        _textFieldThird.frame = CGRectMake(marginX, marginY + _textFieldSec.frame.origin.y+_textFieldSec.frame.size.height, self.contentWidth-2*marginX, 44);
        _textFieldThird.textColor = [UIColor blackColor];
        _textFieldThird.font = [UIFont systemFontOfSize:15];
        _textFieldThird.borderStyle = UITextBorderStyleRoundedRect;
        _textFieldThird.clearButtonMode = UITextFieldViewModeWhileEditing;

        [_textFieldThird addTarget:self action:@selector(clickTextChanged:) forControlEvents:UIControlEventEditingChanged];
    }
    return _textFieldThird;
}
