...
@class GCDBAsyncReadPacket;
@class GCDBAsyncWritePacket;
@class GCDBAsyncSocketPreBuffer;

extern NSString *const GCDBAsyncSocketException;
extern NSString *const GCDBAsyncSocketErrorDomain;

extern NSString *const GCDBAsyncSocketQueueName;
extern NSString *const GCDBAsyncSocketThreadName;

extern NSString *const GCDBAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const GCDBAsyncSocketUseCFStreamForTLS;
#endif
#define GCDBAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define GCDBAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define GCDBAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const GCDBAsyncSocketSSLPeerID;
extern NSString *const GCDBAsyncSocketSSLProtocolVersionMin;
extern NSString *const GCDBAsyncSocketSSLProtocolVersionMax;
extern NSString *const GCDBAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const GCDBAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const GCDBAsyncSocketSSLCipherSuites;
#if !TARGET_OS_IPHONE
extern NSString *const GCDBAsyncSocketSSLDiffieHellmanParameters;
#endif

#define GCDBAsyncSocketLoggingContext 65535


typedef NS_ENUM(NSInteger, GCDBAsyncSocketError) {
	GCDBAsyncSocketNoError = 0,           // Never used
	GCDBAsyncSocketBadConfigError,        // Invalid configuration
	GCDBAsyncSocketBadParamError,         // Invalid parameter was passed
	GCDBAsyncSocketConnectTimeoutError,   // A connect operation timed out
	GCDBAsyncSocketReadTimeoutError,      // A read operation timed out
	GCDBAsyncSocketWriteTimeoutError,     // A write operation timed out
	GCDBAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	GCDBAsyncSocketClosedError,           // The remote peer closed the connection
	GCDBAsyncSocketOtherError,            // Description provided in userInfo
};
...
@interface GCDBAsyncSocket : NSObject

/**
 * GCDBAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, GCDBAsyncSocket will automatically create it's own socket queue.
 ...
 * 
 * For accepting incoming connections, this means GCDBAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means GCDBAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, GCDBAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, GCDBAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of GCDBAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a GCDBAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a GCDBAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that GCDBAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - GCDBAsyncSocketManuallyEvaluateTrust
 ...
 *     So GCDBAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - GCDBAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default GCDBAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct GCDBAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, GCDBAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by GCDBAsyncSocket,
 ...
 *
 * - GCDBAsyncSocketSSLProtocolVersionMin
 * - GCDBAsyncSocketSSLProtocolVersionMax
 ...
 *
 * - GCDBAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see GCDBAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use GCDBAsyncSocketSSLProtocolVersionMin & GCDBAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * GCDBAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(GCDBAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(GCDBAsyncSocket *)sock didAcceptNewSocket:(GCDBAsyncSocket *)newSocket
 * {
 ...
 * Example usage:
 * 
 * - (void)socket:(GCDBAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
 * {
...
@protocol GCDBAsyncSocketDelegate
@optional
 ...
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(GCDBAsyncSocket *)sock didConnectToUrl:(NSURL *)url;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)socket:(GCDBAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(GCDBAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(GCDBAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(GCDBAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(GCDBAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(GCDBAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(GCDBAsyncSocket *)sock;
 ...
 * 
 * Note: If the GCDBAsyncSocket instance is deallocated while it is still connected,
...
**/
- (void)socketDidDisconnect:(GCDBAsyncSocket *)sock withError:(NSError *)err;
...
**/
- (void)socket:(GCDBAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
